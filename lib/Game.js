		...
		if (this.cannon.hasAmmo === true ) {
			for (let i = 0; i < this.cannon.firedArray.length; i++) {
				...
				if (this.cannon.firedArray[i].hasArrived === true) {
					// const explosion = new Explosion(this.cannon.firedArray[i].targetX, this.cannon.firedArray[i].targetY, 10); 
					this.explosion.draw(this.context, this.cannon.firedArray[i].targetX, this.cannon.firedArray[i].targetY);
