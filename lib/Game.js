...
module.exports = class Game {
  ...
  populateBases() {
    for (let i = 0; i < this.baseXArray.length; i++) {
      let x = this.baseXArray[i];
  ...
  meteorsPickTarget() {
    const targetArray = this.baseXArray.map(function(base) {
      return base + 15;
    });
  ...
  meteorsExploding(i) {
    if (this.meteorArray[i].hasArrived === true) {
      this.explosionArray.push(this.meteorArray[i].explode());
  ...
  manageMissiles() {
    if (this.cannon.hasAmmo === true ) {
      for (let i = 0; i < this.cannon.firedArray.length; i++) {
        ...
        this.cannon.firedArray[i].drawTrail(this.context);
  ...
  missileExplode(i) {
    if (this.cannon.firedArray[i].hasArrived === true) {
      this.explosionArray.push(this.cannon.firedArray[i].explode());
